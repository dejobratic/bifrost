...
class _RemoteExecutor:
    def run_remote(self, setup, command, capture=True):
        return run_remote(setup, command, capture=capture)


class _GitOps:
    def fetch_and_checkout(self, setup, ref, latest=False):
        return fetch_and_checkout(setup, ref, latest=latest)


@app.command()
def run(
    setup: Optional[str] = typer.Option(None, "--setup", "-s", help="Target setup name"),
    ref: Optional[str] = typer.Option(None, "--ref", "-r", help="Git ref (branch/tag/commit) to checkout"),
    latest: bool = typer.Option(False, "--latest", "-l", help="Fetch latest changes before running"),
    force: bool = typer.Option(False, "--force", "-f", help="Skip CI gate check"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be done without executing"),
    command: Optional[list[str]] = typer.Argument(None, help="Command to run remotely (after --)"),
) -> None:
    ...
    console.print(f"[green]Run completed[/green] on {metadata.setup} (run: {metadata.run_id})")
