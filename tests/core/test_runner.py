...
def executor():
    ex = MagicMock()
    ex.run_remote.return_value = subprocess.CompletedProcess(args=[], returncode=0, stdout="ok", stderr="")
    return ex
...
@pytest.fixture
def runner(config, ci_gate, executor, git_ops, copier):
    return Runner(config, ci_gate, executor, git_ops, copier)


class TestResolveSetup:
    def test_resolves_by_name(self, runner, setup_a):
        # Act
    ...
    def test_falls_back_to_default(self, runner, setup_a):
        # Act
    ...
    def test_raises_when_no_name_and_no_default(self, config, ci_gate, executor, git_ops, copier):
        # Arrange
    ...
    def test_raises_for_unknown_setup(self, runner):
        # Act / Assert
...
class TestRun:
    def test_successful_run_with_command(self, runner, executor, copier):
        # Act
    ...
    def test_uses_default_runner_when_no_command(self, runner, executor):
        # Act
    ...
    def test_raises_when_no_command_and_no_runner(self, runner):
        # Act / Assert
        with pytest.raises(ConfigError, match="No command provided"):
            runner.run(setup_name="office-b")

    def test_checks_ci_gate(self, runner, ci_gate):
        # Arrange
    ...
    def test_force_skips_ci_gate(self, runner, ci_gate, executor):
        # Arrange
        ci_gate.is_busy.return_value = True
    ...
    def test_dry_run_does_not_execute(self, runner, executor, git_ops, copier):
        # Act
        meta = runner.run(setup_name="office-a", command=["pytest"], ref="main", dry_run=True)
    ...
    def test_handles_ref_with_latest(self, runner, git_ops):
        # Act
    ...
    def test_raises_on_remote_failure(self, runner, executor, copier):
        # Arrange
        executor.run_remote.return_value = subprocess.CompletedProcess(args=[], returncode=1, stdout="", stderr="fail")
